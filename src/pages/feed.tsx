...
function createArrayOfArrays<T> (bigArr: Array<T>, size: number) {
  const arr = [];
  while (bigArr.length) {
...
const FeedPage = () => {
  ...
  const { wsOrdersConnected, orders, total, totalToday } = useAppSelector(state => state.orders);
  ...
  const size = 10;
  const arrayOfReadyArrays = useMemo(() => createArrayOfArrays<TOrder>(ready, size), [ready]);
  const arrayOfPendingArrays = useMemo(() => createArrayOfArrays<TOrder>(pending, size), [pending]);
